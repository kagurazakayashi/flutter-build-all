...
一个 Dart CLI 工具，放在 Flutter 项目根目录或作为 Git 子模块使用，一键编译到所有支持的平台，并自动附带资源文件和安装脚本。
...
|------|------|--------|------|
| `--test` | 标志 | — | 仅检测环境，不构建 |
| `--name` | 字符串 | 自动读取 pubspec.yaml | 自定义输出目录名称 |
| `--target` | 字符串 | 全部可用平台 | 逗号分隔的平台列表，如 `"windows,linux,web"` |
| `--appver` | 字符串 | 自动读取 pubspec.yaml | 应用版本号，影响输出目录命名 |
| `--web-renderer` | 字符串 | `auto` | Web 渲染器：`auto` / `canvaskit` / `html` |
| `--no-web` | 标志 | — | 跳过 Web 平台 |
| `--skip-analyze` | 标志 | — | 跳过 `flutter analyze` |
| `--jobs` | 整数 | 串行 | 并行任务数。`0` = 自动按 CPU 核心数 |
...
|------|------|--------|------|
| `--appdesc` | 字符串 | 空 | 应用描述 |
| `--appgeneric` | 字符串 | 空 | 通用名称 |
| `--appcategory` | 字符串 | `Utility` | Linux 桌面分类，多个用分号分隔 |
| `--appicon` | 字符串 | `web/icons/Icon-192.png` | 图标路径 |
| `--appidentifier` | 字符串 | 自动推导 | macOS Bundle Identifier |
| `--appmacoscategory` | 字符串 | `public.app-category.utilities` | macOS 应用分类 |

### 路径参数

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `--project-dir` | 字符串 | 当前目录 | 指定 Flutter 项目根目录 |

## 编译流程

1. 读取 `pubspec.yaml`，获取应用名和版本号
2. 检测 `lib/l10n/app_*.arb`，若存在则执行 `flutter gen-l10n`
3. 执行 `flutter analyze`（可用 `--skip-analyze` 跳过）
4. 根据当前 OS 枚举可编译平台
