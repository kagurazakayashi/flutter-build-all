...
void copyDirectory(String src, String dst) {
  for (final FileSystemEntity entity in Directory(src).listSync()) {
    final String target = p.join(dst, p.basename(entity.path));
...
void mergeDirectory(String src, String dst) {
  for (final FileSystemEntity entity
      in Directory(src).listSync(recursive: true)) {
    if (entity is! File) continue;
    final String rel = p.relative(entity.path, from: src);
    final String target = p.join(dst, rel);
