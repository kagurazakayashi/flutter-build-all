...
String get scriptDir {
  final String scriptPath =
      File(Platform.script.toFilePath()).absolute.path;
  final String libDir = p.dirname(scriptPath); // lib/src 或 bin
...
File? findTemplate(String scriptDirPath) {
  final File f = File(scriptDirPath);
  if (f.existsSync()) return f;
  final String cwdPath =
      p.join(Directory.current.path, p.basename(scriptDirPath));
  final File cwd = File(cwdPath);
